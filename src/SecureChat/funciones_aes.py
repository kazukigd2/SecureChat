...
#Modo CBC
def iniciarAES_CBC_cifrado(key_16):
    """Inicia el engine de cifrado AES CBC"""
    iv_16 = get_random_bytes(16)  # IV debe ser de 16 bytes para AES
    aes_cifrado = AES.new(key_16, AES.MODE_CBC, iv=iv_16)
    return aes_cifrado, iv_16

def iniciarAES_CBC_descifrado(key_16, iv_16):
    """Inicia el engine de descifrado AES CBC"""
    aes_descifrado = AES.new(key_16, AES.MODE_CBC, iv=iv_16)
    return aes_descifrado

def cifrarAES_CBC(aes_cifrado, datos):
    """Cifra los datos (bytes) usando AES CBC con padding"""
    datos_padded = pad(datos.encode("utf-8"), AES.block_size)  # Rellenamos para múltiplos de 16 bytes
    datos_cifrado = aes_cifrado.encrypt(datos_padded)
    return datos_cifrado

def descifrarAES_CBC(aes_descifrado, datos_cifrado):
    """Descifra los datos (bytes) usando AES CBC y quita el padding"""
    datos_padded = aes_descifrado.decrypt(datos_cifrado)
    try:
        datos_claro = unpad(datos_padded, AES.block_size)
        return datos_claro
    except ValueError:
        return False